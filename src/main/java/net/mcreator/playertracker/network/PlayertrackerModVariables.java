...
public class PlayertrackerModVariables {
	@SubscribeEvent
	public static void init(FMLCommonSetupEvent event) {
		PlayertrackerMod.addNetworkMessage(PlayerVariablesSyncMessage.class, PlayerVariablesSyncMessage::buffer, PlayerVariablesSyncMessage::new, PlayerVariablesSyncMessage::handler);
	}
	...
	public static class EventBusVariableHandlers {
		...
		public static void clonePlayer(PlayerEvent.Clone event) {
			...
			clone.last_login = original.last_login;
			clone.last_disconnect = original.last_disconnect;
	...
	private static class PlayerVariablesProvider implements ICapabilitySerializable<Tag> {
		...
		public static void onAttachCapabilities(AttachCapabilitiesEvent<Entity> event) {
			if (event.getObject() instanceof Player && !(event.getObject() instanceof FakePlayer))
				event.addCapability(new ResourceLocation("playertracker", "player_variables"), new PlayerVariablesProvider());
		}
	...
	public static class PlayerVariables {
		public double last_login = 0;
		public double last_disconnect = 0;
		public double been_online = 0;
		...
		public void syncPlayerVariables(Entity entity) {
			if (entity instanceof ServerPlayer serverPlayer)
				PlayertrackerMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> serverPlayer), new PlayerVariablesSyncMessage(this));
		}

		public Tag writeNBT() {
			CompoundTag nbt = new CompoundTag();
			nbt.putDouble("last_login", last_login);
			nbt.putDouble("last_disconnect", last_disconnect);
			nbt.putDouble("been_online", been_online);
		...
		public void readNBT(Tag Tag) {
			...
			last_login = nbt.getDouble("last_login");
			last_disconnect = nbt.getDouble("last_disconnect");
			been_online = nbt.getDouble("been_online");
		...
		public static void handler(PlayerVariablesSyncMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
			...
			context.enqueueWork(() -> {
				if (!context.getDirection().getReceptionSide().isServer()) {
					...
					variables.last_login = message.data.last_login;
					variables.last_disconnect = message.data.last_disconnect;
					variables.been_online = message.data.been_online;
